...
export const bookingController = {
  createBooking: async (req: Request, res: Response) => {
    const {
      bookingInfo,
      paymentInfo,
    }: { bookingInfo: BookingInfo; paymentInfo: PaymentInfo } = req.body;
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Invalid date format or range. Dates should be YYYY-MM-DD and start date must be before end date.",
      });
    }
  ...
  checkAvailability: async (req: Request, res: Response) => {
    ...
    if (!start || !end || !roomId) {
      return res.status(400).json({
        message:
          "Parameters start (YYYY-MM-DD), end (YYYY-MM-DD), and roomId are required.",
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Invalid date format or roomId. Dates should be YYYY-MM-DD and roomId should be a number.",
      });
    }
