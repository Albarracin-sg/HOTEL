...
interface PaymentGatewayModalProps {
  onSuccess: (paymentData: any) => void; // Now always passes data
  onCancel: () => void;
  roomPrice: number;
  bookingDetails: {
    roomId: number | null;
...
const PaymentGatewayModal: React.FC<PaymentGatewayModalProps> = ({ onSuccess, onCancel, roomPrice, bookingDetails }) => {
  ...
  const handlePaymentSubmit = (e: React.FormEvent) => {
    ...
    // 1. Construct the object with all payment and booking details
    const paymentData = {
      payment: {
        cardNumber: cardNumber.replace(/\s+/g, ''), // Send card number without spaces
        cardHolderName: cardHolder,
        expirationDate: expiryDate,
        cvv: cvv,
        amount: roomPrice,
        currency: 'USD', // Currency can be dynamic if needed
        transactionTimestamp: new Date().toISOString(), // Date and time of the transaction
      },
      booking: {
        ...
        // You could add more booking details here if available, e.g., guests, room name, etc.
      }
    };
    ...
    setTimeout(() => {
      setProcessing(false);
      // Here, you would validate the card and process the actual payment
      if (cardNumber && cardHolder && expiryDate && cvv) {
        alert(t("payment.payment_success") );
        onSuccess(paymentData); // Call the success function with the full payment data
      } else {
  ...
  const getCardType = (number: string) => {
    ...
    return t("payment.card"); // Default translation for 'Card'
  };
                ...
                <div className="space-y-4 md:space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        type="text" // Can be "password" for masking, but "text" is common for CVV during input
                        id="cvv"
                        value={cvv}
                        onChange={(e) => setCvv(e.target.value.replace(/\D/g, ''))} // Remove non-digits
                        placeholder="123"
                        maxLength={4} // Amex CVV can be 4 digits
                        className="w-full px-4 py-3 bg-gray-700/50 border border-gray-600/50 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-yellow-400/50 focus:border-yellow-400/50 transition-all duration-200"
                        required
                      />
                    </div>
